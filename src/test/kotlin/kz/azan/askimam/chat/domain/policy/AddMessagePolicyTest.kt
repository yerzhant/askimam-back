...
class AddMessagePolicyTest : ChatFixtures() {
    ...
    internal fun `should allow add a new message by an imam`() {
        fixtureClock()
        assertThat(forImam.isAllowed(fixtureChat(), User(fixtureImamId, Imam))).isTrue
    }

    @Test
    internal fun `should not allow add a new message as it's not an imam`() {
        fixtureClock()
        assertThat(forImam.isAllowed(fixtureChat(), User(fixtureImamId, Inquirer))).isFalse
    }

    @Test
    internal fun `should allow an inquirer to add a message to his own chat`() {
        fixtureClock()
        assertThat(forInquirer.isAllowed(fixtureChat(), User(fixtureInquirerId, Inquirer))).isTrue
    }

    @Test
    internal fun `should not allow an inquirer to add a message to someone else's chat`() {
        fixtureClock()
        assertThat(forInquirer.isAllowed(fixtureChat(), User(Id(3), Inquirer))).isFalse
    }
}